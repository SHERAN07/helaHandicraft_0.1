    ...
    if (sizeof($_FILES) == 1) {
        ...
        if (isset($_FILES["img0"])) {
            ...
            if (in_array($file_extension, $allowed_image_extensions)) {
                ...
                $targetDir = __DIR__ . "/../resources/userProfile/";
                if (!is_dir($targetDir)) {
                ...
                $relative_path = "resources/userProfile/" . $file_name;


                $rs = Database::search("SELECT * FROM userProfile WHERE users_email ='" . $email . "'");

                if ($rs->num_rows == 0) {

                    Database::iud("INSERT INTO userProfile (`path`,users_email) VALUES ('" . $relative_path . "','" . $email . "')");
                } else {
                    Database::iud("UPDATE userProfile SET `path`= '" . $relative_path . "' WHERE users_email ='" . $email . "' ");
                }
                echo ("success");
            } else {
